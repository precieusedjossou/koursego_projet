...
interface CommandeRow {
  id: string; client: string; coursier: string; type: string;
  adresse: string; montant: number; commission: number;
  statut: StatutDemande; date: string;
}

const MOCK: CommandeRow[] = [
  { id: 'CMD-001', client: 'Jean Dupont', coursier: 'Moussa E.', type: 'achat', adresse: 'Fidjrossè, Cotonou', montant: 17500, commission: 1000, statut: 'en_cours', date: '2025-04-18 14:30' },
  { id: 'CMD-002', client: 'Sylvie Koffi', coursier: 'Roméo D.', type: 'recuperation_colis', adresse: 'Akpakpa, Cotonou', montant: 8200, commission: 400, statut: 'terminee', date: '2025-04-17 10:12' },
  { id: 'CMD-003', client: 'Kodjo Mensah', coursier: 'N/A', type: 'achat', adresse: 'Gbèdjromèdé, Cotonou', montant: 5000, commission: 0, statut: 'en_attente', date: '2025-04-17 09:00' },
  { id: 'CMD-004', client: 'Marie Agossou', coursier: 'Yaovi M.', type: 'achat', adresse: 'Cadjèhoun, Cotonou', montant: 12000, commission: 600, statut: 'terminee', date: '2025-04-16 16:45' },
  { id: 'CMD-005', client: 'Jean Dupont', coursier: 'N/A', type: 'recuperation_colis', adresse: 'Fidjrossè, Cotonou', montant: 3000, commission: 0, statut: 'annulee', date: '2025-04-15 11:20' },
];
...
export default function CommandesPage() {
  ...
  const totalRevenu = MOCK.filter((c) => c.statut === 'terminee').reduce((s, c) => s + c.commission, 0);

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={h1Style}>Commandes</h1>
        <p style={subtitleStyle}>Suivi de toutes les courses passées sur la plateforme</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 24 }}>
        {(Object.entries(STATUT_CONFIG) as [StatutDemande, typeof STATUT_CONFIG[StatutDemande]][]).map(([key, cfg]) => (
          <div key={key} style={miniCardStyle}>
            <div style={{ fontSize: 24, fontWeight: 800, color: cfg.color }}>{MOCK.filter((c) => c.statut === key).length}</div>
      ...
      <div style={{ background: '#FF8C00', borderRadius: 16, padding: '16px 24px', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 13 }}>Revenus plateforme (commandes terminées)</div>
          <div style={{ color: 'white', fontSize: 28, fontWeight: 800, marginTop: 4 }}>{totalRevenu.toLocaleString()} FCFA</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher par ID ou client..." style={{ ...inputStyle, paddingLeft: 38 }} />
        </div>
      ...
      <div style={tableCardStyle}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #F0F0F0' }}>
              {['ID', 'Client', 'Coursier', 'Type', 'Adresse', 'Montant', 'Commission', 'Statut', 'Date', ''].map((h) => (
            ...
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom: '1px solid #F8F8F8' }}>
                  <td style={tdStyle}><span style={{ fontFamily: 'monospace', fontSize: 12, color: '#FF8C00', fontWeight: 600 }}>{c.id}</span></td>
                  <td style={tdStyle}>{c.client}</td>
                  <td style={tdStyle}><span style={{ color: c.coursier === 'N/A' ? '#9CA3AF' : '#1A1A1A' }}>{c.coursier}</span></td>
                  <td style={tdStyle}>
                    <span style={{ fontSize: 12 }}>{c.type === 'achat' ? '🛒 Achat' : '📦 Colis'}</span>
                  </td>
                  <td style={{ ...tdStyle, maxWidth: 140 }}><span style={{ fontSize: 12, color: '#6B7280' }}>{c.adresse}</span></td>
                  <td style={tdStyle}><strong>{c.montant.toLocaleString()} FCFA</strong></td>
                  <td style={tdStyle}><span style={{ color: c.commission > 0 ? '#16A34A' : '#9CA3AF', fontWeight: 600 }}>{c.commission > 0 ? `+${c.commission} FCFA` : '—'}</span></td>
                  <td style={tdStyle}>
                    <span style={{ background: cfg.bg, color: cfg.color, padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' }}>{cfg.label}</span>
                  </td>
                  <td style={{ ...tdStyle, fontSize: 12, color: '#9CA3AF', whiteSpace: 'nowrap' }}>{c.date}</td>
                  <td style={tdStyle}>
                    <button style={iconBtnStyle} title="Voir le détail"><Eye size={14} /></button>
                  </td>
                </tr>
              );
            })}
        ...
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '40px 0', color: '#9CA3AF' }}>Aucune commande trouvée</div>}
      </div>
    </div>
  );
}
